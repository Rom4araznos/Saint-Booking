CREATE OR REPLACE FUNCTION select_discounted_hotels_start(
	...
	_country text,
	_city text        ) 
        RETURNS TABLE(name character varying(60), discount numeric(3,2), country character varying(30), city character varying(30), user_rating numeric(2,1), photos_path text) AS $$
    BEGIN
         RETURN QUERY
        	SELECT hotels.name , MAX(discounted_hotels.discount) AS discount, hotels.country, hotels.city, hotels.user_rating, h_photos_path
		FROM hotels

CREATE OR REPLACE FUNCTION select_hotels_start(
	...
	_city text
        ...
        RETURNS TABLE(name character varying(60), price numeric(10,2), country character varying(30), city character varying(30),user_rating numeric(2,1) ,photos_path text) AS $$
    BEGIN
         RETURN QUERY
        	SELECT hotels.name, MIN(room.price) AS price,hotels.country, hotels.city,hotels.user_rating, hotels_photos.h_photos_path
		FROM hotels
		...
		GROUP BY hotels.name,hotels.country, hotels.city, hotels.stars,hotels.user_rating,h_photos_path
		ORDER BY 
    CASE 
